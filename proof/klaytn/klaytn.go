...
func GetProof(client *ethclient.Client, kClient *Client, latestBlock *big.Int, log *types.Log, method string, fId constant.ChainId) ([]byte, error) {
...
func AssembleProof(cli *Client, header Header, log types.Log, fId constant.ChainId, receipts []*types.Receipt, method string) ([]byte, error) {
	...
	input, err := constant.Klaytn.Methods[constant.MethodOfGetBytes].Inputs.Pack(pd)
	if err != nil {
	...
	input, err = constant.Klaytn.Methods[constant.MethodOfGetFinalBytes].Inputs.Pack(finpd)
	if err != nil {
