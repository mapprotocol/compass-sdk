...
func AssembleProof(block *UpdateBlock, log types.Log, receipts []*types.Receipt, method string, fId constant.ChainId) ([]byte, error) {
	txIndex := log.TxIndex
	...
	input, err := constant.Platon.Methods[constant.MethodOfGetBytes].Inputs.Pack(ProofData{
		ReceiptProof: rp,
		Header:       block.Header,
